...
module UDDF
  class TestDocument < Minitest::Test
    def setup
      @uddf_regex = %r{<uddf version="#{Document::VERSION}">.*<generator>.*</generator>.*</uddf>}m
    end

    def test_document
      assert_instance_of Document, Document.new, "UDDF::Document.new does not produce a valid object"
    end
  ...
  class TestGenerator < Minitest::Test
    ...
    def test_valid
      assert @gen.valid?, "Generator does not produce a valid instance"
    end

    def test_datetime_is_iso8601_format
      # 2014-06-09T20:39:34-04:00"
      assert_match(/-\d{2}T\d+:/, @gen.nodes[:datetime], "UDDF::Generator doesn't return ISO8601 time format")
    end

    def test_generator_nodes
      assert_equal %i[name type version datetime], @gen.nodes.keys
    end

    def test_to_elem
      assert_respond_to @gen, :to_elem, "UDDF::Generator#to_elem is not valid"
